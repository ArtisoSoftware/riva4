...
const uploadFile1 = async () => {
  if (file1.value) {
    ...
    try {
      $q.loading.show();
      let url = serverData.value + "cc/strupload1.php";
...
const confirmDeletion1 = async () => {
  $q.loading.show();
  let url = serverData.value + "cc/strdelete1.php";
  let dataSend = {
...
const confirmClear1 = async () => {
  $q.loading.show();
  let res2 = await axios.get(serverData.value + "cc/strclear1.php");
  $q.loading.hide();
...
const makeIndex1 = async () => {
  $q.loading.show();
  let res2 = await axios.get(serverData.value + "cc/strmakeindex1.php");
  $q.loading.hide();
  if (res2.data == "finish") {
    Notify.create({

      ...
      <q-dialog v-model="confirmClearData1">
        <q-card>
          <q-card-section>
            <div class="text-h6">Confirm Deletion</div>
            <div>
              Do you want to delete all data from GVC top 5 partner economies?
            </div>
          </q-card-section>
...
const uploadFile1 = async () => {
  if (file1.value) {
    ...
    try {
      let url = serverData.value + "cc/gvcupload3.php";
...
const confirmClear1 = async () => {
  let res2 = await axios.get(serverData.value + "cc/gvcclear3.php");

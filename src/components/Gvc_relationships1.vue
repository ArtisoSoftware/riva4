    ...
    <div style="width: 1000px; margin: auto">
      ...
      <q-dialog v-model="confirmClearData1">
        <q-card>
          <q-card-section>
            <div class="text-h6">Confirm Deletion</div>
            <div>Do you want to delete all data from GVC overview?</div>
          </q-card-section>
...
const uploadFile1 = async () => {
  if (file1.value) {
    ...
    try {
      let url = serverData.value + "cc/gvcupload1.php";
...
const confirmDeletion1 = async () => {
  let url = serverData.value + "cc/gvcdelete1.php";
  let dataSend = {
...
const confirmClear1 = async () => {
  let res2 = await axios.get(serverData.value + "cc/gvcclear1.php");
...
const makeIndex1 = async () => {
  let res2 = await axios.get(serverData.value + "cc/gvcmakeindex1.php");
  if (res2.data == "finish") {
    Notify.create({
